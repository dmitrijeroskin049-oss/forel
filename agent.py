...
# Отчёты рыбаков (где/на что/когда ловят) анализируем с этой даты.
REPORT_START = "2026-09-01"

# ============================================================
# LLM (опционально). Любой OpenAI-совместимый API.
#   export LLM_API_KEY="sk-..."
#   export LLM_API_URL="https://openrouter.ai/api/v1/chat/completions"
#   export LLM_MODEL="openai/gpt-4o-mini"
#   export LLM_MAX_POSTS="40"
# Без ключа блок «Когда клюёт» будет отключён, остальное работает.
# ============================================================
...
# Намёк на время суток — кандидат для LLM-анализа
...
try {
    ...
    /* --- LLM: когда клюёт --- */
    const LT = D.llm_time || {};
    if (LT.enabled && LT.labels && LT.labels.length > 0) {
        ...
        document.getElementById('llmnote').textContent = LT.enabled
            ? 'Пока нет проанализированных отчётов — данные появятся после следующих запусков.'
            : 'LLM-анализ отключён. Задайте переменную окружения LLM_API_KEY и перезапустите сборку.';
...
def llm_chat(prompt_text):
    """Один запрос к OpenAI-совместимому API. Возвращает текст ответа или None."""
    try:
...
def parse_llm_json(raw):
    """Достаёт JSON из ответа модели (убирает markdown-обёртки)."""
    if not raw:
        return None
...
def run_llm_time_analysis(database, candidates):
    """
    Анализирует отчёты через LLM с кэшированием в SQLite.
    candidates: список (post_id, day, text).
    Возвращает агрегат {"утро": {"bite": n, "no_bite": m}, ...} и кол-во постов.
    """
    database.execute(
        """
    ...
    if LLM_API_KEY and new_posts:
        ...
        print(f"LLM: анализирую {len(to_analyze)} новых отчётов "
              f"(в кэше {len(cached)}, лимит за запуск {LLM_MAX_POSTS})...")
        ...
        print("LLM: LLM_API_KEY не задан — использую только кэш "
...
def build(database, state, last_page):
    ...
    for post_id, page_number, author, post_datetime, text in database.execute(query):
        ...
        # --- Отчёты рыбаков (где/на что) и кандидаты для LLM ---
    ...
    # --- LLM: когда клюёт ---
    llm_candidates.sort(key=lambda c: c[1], reverse=True)  # свежие в приоритете
    ...
    print(f"Сайт собран: остаток {remaining} кг, отчётов: {len(reports)}, LLM-анализов: {llm_analyzed}")
