...
# --- FIX: принимаем оба имени секрета ---
LLM_API_KEY = (os.getenv("LLM_API_KEY") or os.getenv("OPENAI_API_KEY") or "").strip()
LLM_BASE_URL = (os.getenv("LLM_BASE_URL") or os.getenv("OPENAI_BASE_URL") or "https://api.openai.com/v1").strip()
LLM_MODEL = (os.getenv("LLM_MODEL") or os.getenv("OPENAI_MODEL") or "gpt-4o-mini").strip()
...
<p>🎣 2 снасти, до 2 крючков. 👩 Женщина и ребёнок до 13 лет бесплатно. 🐟 Нормы вылова нет.</p>
<div class="note">Источник: <a id="cond_link" href="#" target="_blank">администрация</a> от <span id="cond_date">—</span>. Авто-парсится.</div>
</div></details>
...
<div class="card"><h3>По месяцам</h3><div class="chart-box"><canvas id="m"></canvas></div></div>
<div class="card"><h3>Клёв vs Давление</h3><div class="chart-box"><canvas id="p"></canvas></div></div>
<div class="card"><h3>Клёв vs Фаза луны</h3><div class="chart-box"><canvas id="moonChart"></canvas></div></div>
<div class="card"><h3>Последние активные дни — день/ночь + ветер + луна</h3><div class="table-wrap"><table id="t"></table></div></div>
</details>
<details open><summary>⏰ Когда клюёт (анализ отчётов LLM)</summary>
<div class="card"><div class="chart-box"><canvas id="timeChart"></canvas></div><div class="note" id="timeMethod" style="margin-top:8px;color:#fbbf24"></div><div class="note">Каждый отчёт рыбака прочитан языковой моделью: «утром тишина, вечером раздача» учитывается корректно — утро попадает в «не клевало», вечер в «клевало».</div></div>
</details>
<details open><summary>🎣 Разбор отчётов с сентября 2026</summary>
...
if(D.table&&D.table.length>0){
 document.getElementById('t').innerHTML='<tr><th>Дата</th><th>П</th><th>День</th><th>Ночь</th><th>Давл</th><th>Ветер</th><th>Луна</th><th></th></tr>'+D.table.map(r=>{
  const wind=r.wind_speed!=null?`${r.wind_speed} м/с ${r.wind_gust!=null?'/ '+r.wind_gust:''} ${r.wind_dir_str||''}`:'—';
  const moon=r.moon?`${r.moon.icon} ${r.moon.name}`:'—';
  return `<tr><td>${r.day}</td><td>${r.posts}</td><td>${r.temp_max??'—'}°</td><td>${r.temp_min??'—'}°</td><td>${r.pressure??'—'}</td><td>${wind}</td><td>${moon}</td><td>${(r.links||[]).map((u,i)=>`<a href="${u}" target="_blank">#${i+1}</a>`).join(' ')}</td></tr>`;
 }).join('');
}
if(D.time_bite){
 document.getElementById('timeMethod').textContent=D.time_bite_method||'';
...
if(D.report_stats){
 const tl=D.report_stats.top_locations||{}; document.getElementById('top_locs').innerHTML='<tr><th>Точка</th><th>упом.</th></tr>'+Object.entries(tl).map(([k,v])=>`<tr><td>${k}</td><td>${v}</td></tr>`).join('');
 const tu=D.report_stats.top_lures||{}; document.getElementById('top_lures').innerHTML='<tr><th>Приманка</th><th>упом.</th></tr>'+Object.entries(tu).map(([k,v])=>`<tr><td>${k}</td><td>${v}</td></tr>`).join('');
}
if(D.reports){document.getElementById('reports').innerHTML='<tr><th>Дата</th><th>Автор</th><th>Точка</th><th>Приманка</th><th>Время</th><th>Улов</th><th>Цитата</th></tr>'+D.reports.map(r=>`<tr><td>${r.day}</td><td>${r.author||'—'}</td><td>${r.location||'—'}</td><td>${r.lure||'—'}</td><td>${r.time||'—'}</td><td>${r.catch||'—'}</td><td class="q"><a href="${r.url}" target="_blank">${r.quote}</a></td></tr>`).join('');}
...
def llm_analyze_time(reports):
    ...
    agg={}; base=LLM_BASE_URL.rstrip("/")
    print(f"LLM enabled={USE_LLM} model={LLM_MODEL} base={base} reports={len(reports)}")
    for i in range(0,len(reports),12):
        ...
        try:
            resp=requests.post(f"{base}/chat/completions",headers={"Authorization":f"Bearer {LLM_API_KEY}","Content-Type":"application/json"},json={"model":LLM_MODEL,"messages":[{"role":"user","content":prompt}],"temperature":0.1,"max_tokens":1000},timeout=90)
            j=resp.json()
            if "error" in j: print(f"LLM API error: {j['error']}"); continue
            content=j["choices"][0]["message"]["content"]
            m=re.search(r"\[.*\]",content,re.S)
            if not m: print(f"LLM no json: {content[:200]}"); continue
...
def main():
    ...
    state=load_state(); print(f"Проверяю форум... LLM enabled={USE_LLM}")
...
def build(database,state,last_page):
    ...
    if USE_LLM and detailed_full:
        ...
        if llm_res:
            ...
            time_method=f"✅ LLM-анализ включен ({LLM_MODEL}). Каждый отчёт прочитан моделью."
        else:
            for r in detailed_full:
                for tt in extract_biting_times_heuristic(r["text"]): time_counter[tt]+=1
            time_method="⚠️ LLM ключ задан, но ответ не получен — использована эвристика с учетом отрицаний."
    else:
        for r in detailed_full:
            for tt in extract_biting_times_heuristic(r["text"]): time_counter[tt]+=1
        time_method="LLM-анализ отключён. Задайте переменную окружения LLM_API_KEY (или OPENAI_API_KEY) в Secrets и перезапустите сборку. Сейчас работает эвристика с учетом отрицаний."
    ...
    print(f"Сайт собран: остаток {remaining} кг, отчетов с сентября {len(detailed_full)}, время клева: {time_counter} method={time_method}")
